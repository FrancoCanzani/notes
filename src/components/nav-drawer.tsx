...
export default function NavDrawer({ notes }: { notes?: Note[] }) {
  ...
  return (
    <Drawer>
      ...
      <DrawerContent className='bg-quarter-spanish-white-100'>
        <div className='mx-auto w-full max-w-sm'>
          ...
          <div className='flex flex-col justify-between w-full px-1.5 py-4 min-h-60'>
            <div className='space-y-4'>
              ...
              >
                <Input
                  className='text-center focus:text-start bg-quarter-spanish-white-50 border-none placeholder:font-medium focus-visible:ring-0 focus-visible:ring-offset-0 outline-none font-medium text-sm w-full'
                ...
                <Button
                  variant={'menu'}
                  size={'lg'}
                  className='px-3 py-2 h-10 rounded-md font-medium bg-quarter-spanish-white-50'
                >
                  Create
                </Button>
              </form>
              <div>
                {[...pinnedNotes, ...activeNotes].map((note) => (
                  <Link href={`/notes/${note.id}`} key={note._id}>
                    <div
                      className={cn(
                        'p-1.5 my-1 bg-quarter-spanish-white-100 rounded-lg text-sm w-full flex-col items-center justify-between hover:bg-quarter-spanish-white-200',
